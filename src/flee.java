...
public class flee implements Runnable {
    ...
    private void gameLoop() {
		...
		if(Mouse.dragged) {
			if((Mouse.mouseX - 2 > x - 25 && Mouse.mouseX - 2 < x + 25 && Mouse.mouseY - 25 > y - 25 
                                && Mouse.mouseY - 25 < y + 25) || dragging) {
				if(startTime == -1) {
		...
		if(collision()) {
			...
			if(best < ((endTime - startTime) / 500.0)) {
				best = ((endTime - startTime) / 500.0);
	...
	public static void render(Graphics2D g) {
		...
		Color customColor = Color.decode("#F7DC6F");
		...
		if(gameOver) {
			...
			String sg = "Game Over";
                        String st = "Time: " + ((endTime - startTime) / 500.0) + "s";
	...
	public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new flee());
